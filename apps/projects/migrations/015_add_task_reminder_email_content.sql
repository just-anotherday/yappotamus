-- Migration 015: capture task reminder email content at dispatch time.
-- `to_char(date, 'Mon FMDD, YYYY')` formats the DATE directly, without a
-- timestamp or timezone conversion (for example, 2026-08-14 -> Aug 14, 2026).
-- Shopping and recipe email content intentionally remains unchanged: keep any
-- V1 user-facing email opt-in task-only until those targets have contracts.

BEGIN;

DO $migration_015_preflight$
BEGIN
  IF to_regprocedure('public.generate_custom_reminders(timestamp with time zone)') IS NULL THEN
    RAISE EXCEPTION 'Migration 015 requires generate_custom_reminders(timestamptz)';
  ELSIF to_regclass('public.reminder_deliveries') IS NULL THEN
    RAISE EXCEPTION 'Migration 015 requires public.reminder_deliveries';
  ELSIF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'tasks'
      AND column_name = 'due_on' AND udt_name = 'date'
  ) THEN
    RAISE EXCEPTION 'Migration 015 requires public.tasks.due_on DATE';
  ELSIF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'reminder_deliveries'
      AND column_name IN ('subject', 'text_body') AND udt_name = 'text'
    GROUP BY table_schema, table_name HAVING count(*) = 2
  ) THEN
    RAISE EXCEPTION 'Migration 015 requires reminder_deliveries subject and text_body text columns';
  END IF;
END
$migration_015_preflight$;
    ...
    SELECT claimed.id, 'email', 'queued', 0, p_now,
      ...
      CASE
        WHEN claimed.task_id IS NOT NULL THEN format(
          'You asked to be reminded about this task.%s%s%sView task:%shttps://projects.yapvibes.com/?board=projects&project=%s&task=%s',
          E'\n\n',
          left(coalesce(nullif(btrim(claimed.task_title), ''), 'Untitled'), 128),
          CASE WHEN claimed.task_due_on IS NULL THEN E'\n\n' ELSE format('%s%s', E'\n\nDue: ', to_char(claimed.task_due_on, 'Mon FMDD, YYYY')) || E'\n\n' END,
          E'\n', claimed.task_project_id, claimed.task_id
        )
        WHEN claimed.shopping_project_id IS NOT NULL THEN format('You asked to be reminded about this shopping list.%s%s', E'\n\n', left(coalesce(nullif(btrim(claimed.shopping_project_name), ''), 'Untitled'), 128))
        ELSE format('You asked to be reminded about this recipe.%s%s', E'\n\n', left(coalesce(nullif(btrim(claimed.recipe_name), ''), 'Untitled'), 128))
      END
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'Migration 015 postflight failed: generator security mismatch';
  END IF;
END
$migration_015_postflight$;
