...
function renderRoundHistory() {
  ...
  const activePlayer = getSessionPlayerName();
  const filteredHistory = activePlayer
    ? roundHistory.filter((entry) => entry.player === activePlayer)
    : [];

  renderRoundHistoryCapacity(filteredHistory.length);

  if (!activePlayer) {
    const placeholder = document.createElement("li");
    placeholder.textContent = "Set Arcade Tag to start tracking your rounds.";
  ...
  if (filteredHistory.length === 0) {
    const placeholder = document.createElement("li");
    placeholder.textContent = `No rounds yet for ${activePlayer}.`;
    elements.roundHistoryList.appendChild(placeholder);
    return;
  }
